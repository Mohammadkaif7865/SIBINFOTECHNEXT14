...
export default function ParagraphRewriter() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${CONSTANTS.API_URL}chat/paragraph-rewriter`,
        { paragraph, tone, style },
        { headers }
      );
      ...
      if (response.data.success) {
        ...
        setError("Failed to rewrite paragraph. Please try again.");
      }
    } catch (err) {
  ...
  return (
    ...
    <div className="rewriter-container">
      <h1 className="rewriter-title">📝 Paragraph Rewriter Tool</h1>
      <p className="rewriter-subtext">
        Instantly rewrite your paragraph in the tone and structure you prefer.
      </p>

      <form onSubmit={handleSubmit} className="rewriter-form">
        <label className="rewriter-label">Paragraph to rewrite</label>
        <textarea
        ...
        <div className="rewriter-radio-group">
          <label><input type="radio" name="style" value="nochange" checked={style === "nochange"} onChange={() => setStyle("nochange")} /> No change</label>
          <label><input type="radio" name="style" value="shorten" checked={style === "shorten"} onChange={() => setStyle("shorten")} /> Shorten</label>
          <label><input type="radio" name="style" value="expand" checked={style === "expand"} onChange={() => setStyle("expand")} /> Expand</label>
        </div>
      ...
      {result && (
        <div className="rewriter-alert">
          <strong>Rewritten Paragraph:</strong>
